...
!!! info "`ModuleRef`类是从`@nestjs/core`包导入的。"
...
使用`ContextIdFactory`类来生成上下文标识符。
...
!!! info "`ContextIdFactory`类是从`@nestjs/core`包中导入的。"

## 注册请求的提供器

手动生成的上下文标识符(使用`ContextIdFactory.create()`)表示 DI 子树，其中`REQUEST`提供器是`undefined`，因为它们没有被 Nest 依赖注入系统实例化和管理。
...
为了共享相同的 DI 容器子树，你必须获取当前的上下文标识符，而不是生成一个新的上下文标识符(例如，使用`ContextIdFactory.create()`函数，如上所示)。
...
!!! info "了解关于请求提供器的更多信息[此处](https://docs.nestjs.com/fundamentals/injection-scopes#request-provider)."

现在，使用`ContextIdFactory`类的`getByRequest()`方法创建一个基于请求对象的上下文 id，并将其传递给`resolve()`调用:
